...
const importPlugin__namespace = /*#__PURE__*/_interopNamespaceCompat(importPlugin);
...
const { resolveModulePath } = exsolve.createResolver({ extensions: [".json"] });
function loadAutoImports(options = true) {
  ...
  try {
    const autoImportModuleURL = resolveModulePath(node_path.resolve(".", autoImportsFile), { try: true });
...
const currentScope = new URL(".", (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))).pathname;
  ...
  return [
    {
      ...
      ...filesConfig,
      plugins: { import: importPlugin__namespace },
      rules: {
        "import/first": "error",
